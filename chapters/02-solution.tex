...
In contrast, the example in Figure \ref{fig:exe-classifier-binomial} shows how a binomial classifier works: for each of the three classes: \textit{class--a}, \textit{class--b} and \textit{class--c}, an analogical classifier decides whether a given verb belongs to said class or not.
...
With the model in place, we can calculate the predictions of different hierarchy shapes. In a simple tree-like hierarchy, as in Figure \ref{fig:formal-exe-tree}:
...
It is important to note, however, that the semi formalization of the previous section is not a requirement for the thesis of this book. The empirical results presented in Part II are the main contribution of this work. %Seeing analogy as constraints on types is a way of making sense of the close relation between type hierarchies that describe morphological systems, and the analogical relations found in these systems, but alternative explanations are not out of the question.
